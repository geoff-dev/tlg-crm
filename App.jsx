...
const LIST_COLS = "id,job_name,stage,project_type,job_location,lead_source,lead_date,sale_amount,contact_id";
const PIPE_COLS = "id,job_name,stage,estimate_amount,lead_date,contact_id,sale_amount,date_sold";
  ...
  return (<div>
    ...
    <div style={{background:"#fff",borderRadius:14,border:"1px solid #e8e6df",padding:"20px 24px",boxShadow:cardShadow}}>
      ...
      {form.stage==="Lost"&&<><SH>Why was this lead lost?</SH><div style={{display:"flex",flexWrap:"wrap",gap:12}}>
        ...
        <Field label="Stage lost" half><input style={inpS} value={form.stage_lost||""} onChange={set("stage_lost")}/></Field>
        <Field label="Reason" half><input style={inpS} value={form.lost_reason||""} onChange={set("lost_reason")}/></Field>
      </div></>}
...
function ContactDetail({ contact, onBack, onSaved, onOpenProject }) {
  ...
  const handleSave = () => {
    ...
    sbUpdate("contacts", contact.id, { first_name:form.first_name, last_name:form.last_name, email:form.email, phone_home:form.phone_home, phone_cell:form.phone_cell, address:form.address, city:form.city, state:form.state, zip:form.zip, location:form.location, home_value:form.home_value?parseFloat(form.home_value):null })
    .then(() => { setSaving(false); if(onSaved) onSaved(); });
  };
  ...
  return (<div>
    ...
    <div style={{background:"#fff",borderRadius:14,border:"1px solid #e8e6df",padding:"20px 24px",boxShadow:cardShadow}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:12}}>
        ...
        <Field label="Address" half><input style={inpS} value={form.address||""} onChange={set("address")}/></Field>
        <Field label="City" half><input style={inpS} value={form.city||""} onChange={set("city")}/></Field>
        <Field label="State" half><input style={inpS} value={form.state||""} onChange={set("state")}/></Field>
        <Field label="Zip" half><input style={inpS} value={form.zip||""} onChange={set("zip")}/></Field>
        <Field label="Location" half><select style={inpS} value={form.location||""} onChange={set("location")}><option value="">Select...</option>{LOCS.map(l=><option key={l}>{l}</option>)}</select></Field>
...
function PipelineView({ onOpenProject }) {
  ...
  useEffect(() => {
    sbGetCached("projects",`select=${PIPE_COLS}&stage=neq.Lost&order=lead_date.desc.nullslast&limit=2000`).then(all => {
      ...
      setSoldRecent(recentSold.slice(0,10)); setData(grouped); setLoading(false);
...
function NewProjectModal({ onClose, onCreated }) {
  ...
  const [saving, setSaving] = useState(false);
  const [cf, setCf] = useState({first_name:"",last_name:"",email:"",phone_cell:"",phone_home:"",address:"",city:"",state:"IN",zip:""});
  const [pf, setPf] = useState({job_name:"",stage:"Not Yet Contacted",project_type:"",lead_source:"",salesperson:TEAM[0],lead_date:todayStr(),estimate_amount:"",confidence:""});
  ...
  const handleSave = () => { setSaving(true); cacheClear();
    ...
    else { sbInsert("contacts",cf).then(contact=>{ if(!contact){setSaving(false);return;} const jn=pf.job_name||(cf.last_name||"New")+" Residence"; sbInsert("projects",{...pf,job_name:jn,contact_id:contact.id,estimate_amount:pf.estimate_amount?parseFloat(pf.estimate_amount):null,confidence:pf.confidence?parseInt(pf.confidence):null}).then(r=>{setSaving(false);if(r&&onCreated)onCreated(r);}); }); }
  };
  ...
  return (<Modal title="New Project" onClose={onClose} width={620}>
    ...
    {mode==="new"&&<div style={{marginBottom:16}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
        ...
        <Field label="Address" half><input style={inpS} value={cf.address} onChange={setC("address")}/></Field>
        <Field label="City" half><input style={inpS} value={cf.city} onChange={setC("city")}/></Field>
      </div>
    </div>}
...
function NewContactModal({ onClose, onCreated }) {
  const [f, setF] = useState({first_name:"",last_name:"",email:"",phone_cell:"",phone_home:"",address:"",city:"",state:"IN",zip:"",location:"",home_value:""});
  const [saving, setSaving] = useState(false);
  ...
  return (<Modal title="New Contact" onClose={onClose} width={560}>
    <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
      ...
      <Field label="Address" half><input style={inpS} value={f.address} onChange={set("address")}/></Field>
      <Field label="City" half><input style={inpS} value={f.city} onChange={set("city")}/></Field>
      <Field label="State" half><input style={inpS} value={f.state} onChange={set("state")}/></Field>
      <Field label="Zip" half><input style={inpS} value={f.zip} onChange={set("zip")}/></Field>
      <Field label="Location" half><select style={inpS} value={f.location} onChange={set("location")}><option value="">Select...</option>{LOCS.map(l=><option key={l}>{l}</option>)}</select></Field>
...
export default function App() {
  ...
  const loadProjects = useCallback(() => {
    ...
    if (search && search.trim()) {
      ...
      .then(contactResults => {
        ...
        const sf = `&or=(${orParts})`;
        Promise.all([sbGet("projects",params+sf),sbCount("projects",countP+sf)]).then(([r,c])=>{setProjects(r||[]);setTotalCount(c);setLoading(false);});
      });
    } else {
      Promise.all([sbGet("projects",params),sbCount("projects",countP)]).then(([r,c])=>{setProjects(r||[]);setTotalCount(c);setLoading(false);});
    }
  ...
  const tableCols = [{k:"job_name",l:"Job"},{k:"stage",l:"Stage"},{k:"project_type",l:"Type"},{k:"job_location",l:"Location"},{k:"lead_source",l:"Source"},{k:"lead_date",l:"Lead date"},{k:"sale_amount",l:"Sale $"}];
        ...
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            ...
            <tbody>
              ...
              {projects.map(p=><tr key={p.id} onClick={()=>openProject(p)} style={{borderBottom:"1px solid #f0eeea",cursor:"pointer"}} onMouseEnter={e=>{e.currentTarget.style.background="#f7f6f3";}} onMouseLeave={e=>{e.currentTarget.style.background="transparent";}}>
                <td style={{padding:"10px 8px",fontWeight:500}}>{p.job_name||"—"}</td>
