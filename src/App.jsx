...
function ManagementConsole({ onOpenProject }) {
  ...
  useEffect(function() {
    Promise.all([
      sbGet("projects", "select=id,job_name,stage,project_type,job_location,lead_source,lead_date,sale_amount,estimate_amount,estimate_date,date_sold,date_lost,stage_lost,lost_reason,contact_id,salesperson,buying_behavior,confidence&lead_date=gte." + curYear + "-01-01&lead_date=lte." + curYear + "-12-31&limit=50000"),
      ...
      var pipelineByStage = {};
      STAGES.forEach(function(s) { if (s !== "Sold" && s !== "Lost") pipelineByStage[s] = { count: 0, estTotal: 0, weighted: 0 }; });
      ...
      setData({
        ...
        varianceData: varianceData, lostByStage: lostByStage, lostByReason: lostByReason
      });
      setLoading(false);
    });
  }, []);
  ...
  // Pipeline totals
  var totalWeighted = 0; var totalPipeEst = 0;
  Object.values(data.pipelineByStage).forEach(function(s) { totalWeighted += s.weighted; totalPipeEst += s.estTotal; });
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={{overflowX:"auto",border:"0.5px solid #e8e6df",borderRadius:10}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:13,minWidth:800}}>
          ...
          <tbody>{spList.map(function(sp, i) {
            ...
            var rankBg = i === 0 ? "#FFF7E0" : i === 1 ? "#F0F0F0" : i === 2 ? "#FFF0E0" : i % 2 === 0 ? "transparent" : "#f7f6f330";
            var rankBorder = i === 0 ? "#D4A017" : i === 1 ? "#A0A0A0" : i === 2 ? "#CD7F32" : "transparent";
            return <tr key={sp.name} style={{borderBottom:"0.5px solid #f0eeea",background:rankBg,borderLeft:"3px solid "+rankBorder}}>
              <td style={Object.assign({},tdL,{fontWeight:700,color:i<3?"#8B6914":"#b0ada6"})}>{i+1}</td>
    ...
    {section === "pipeline" && <div>
      <div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:16}}>
        <div style={{background:"#f7f6f3",borderRadius:10,padding:"12px 18px",flex:"1 1 160px"}}><div style={{fontSize:11,fontWeight:500,color:"#8a8780",textTransform:"uppercase",letterSpacing:"0.06em"}}>Total pipeline (est.)</div><div style={{fontSize:22,fontWeight:500}}>{fmtC(totalPipeEst)}</div></div>
        <div style={{background:"#E6F1FB",borderRadius:10,padding:"12px 18px",flex:"1 1 160px"}}><div style={{fontSize:11,fontWeight:500,color:"#0C447C",textTransform:"uppercase",letterSpacing:"0.06em"}}>Weighted forecast</div><div style={{fontSize:22,fontWeight:500,color:"#0C447C"}}>{fmtC(totalWeighted)}</div></div>
        <div style={{background:"#EAF3DE",borderRadius:10,padding:"12px 18px",flex:"1 1 160px"}}><div style={{fontSize:11,fontWeight:500,color:"#173404",textTransform:"uppercase",letterSpacing:"0.06em"}}>Revenue closed YTD</div><div style={{fontSize:22,fontWeight:500,color:"#173404"}}>{fmtC(totalRevSold)}</div></div>
        <div style={{background:"#f7f6f3",borderRadius:10,padding:"12px 18px",flex:"1 1 160px"}}><div style={{fontSize:11,fontWeight:500,color:"#8a8780",textTransform:"uppercase",letterSpacing:"0.06em"}}>Projected total</div><div style={{fontSize:22,fontWeight:500}}>{fmtC(totalRevSold + totalWeighted)}</div></div>
      </div>
      <div style={{overflowX:"auto",border:"0.5px solid #e8e6df",borderRadius:10}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
          <thead><tr style={{borderBottom:"2px solid #e8e6df"}}>
            <th style={thL}>Stage</th><th style={thS}>Projects</th><th style={thS}>Total est. $</th><th style={thS}>Weight</th><th style={thS}>Weighted $</th>
          </tr></thead>
          <tbody>{STAGES.filter(function(s){return s!=="Sold"&&s!=="Lost";}).map(function(s,i) {
            var d = data.pipelineByStage[s] || { count: 0, estTotal: 0, weighted: 0 };
            ...
            return <tr key={s} style={{borderBottom:"0.5px solid #f0eeea",background:i%2===0?"transparent":"#f7f6f330"}}>
              ...
              <td style={Object.assign({},tdBold,{color:"#185FA5"})}>{fmtC(d.weighted)}</td>
            </tr>;
          })}</tbody>
          <tfoot><tr style={{borderTop:"2px solid #e8e6df",fontWeight:600}}>
            ...
            <td style={tdS}></td>
            <td style={Object.assign({},tdBold,{color:"#185FA5"})}>{fmtC(totalWeighted)}</td>
          </tr></tfoot>
        </table>
      </div>
      <div style={{marginTop:12,fontSize:12,color:"#8a8780"}}>Weighted forecast = estimate × stage probability. Not Yet Contacted 5% → Prepare To Close 90%.</div>
    </div>}
...
function SalesHistory() {
  ...
  var medalColors = [{bg:"#FFF7E0",border:"#D4A017",label:"#8B6914"},{bg:"#F0F0F0",border:"#A0A0A0",label:"#666"},{bg:"#FFF0E0",border:"#CD7F32",label:"#8B5E14"}];
