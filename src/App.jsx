...
function LoginScreen({ onLogin }) {
  ...
  return (<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#f7f6f3"}}>
    <div style={{background:"#fff",borderRadius:16,padding:"40px 36px",boxShadow:"0 4px 24px rgba(0,0,0,0.08)",maxWidth:360,width:"100%",textAlign:"center"}}>
      <h1 style={{margin:"0 0 4px",fontSize:28,fontWeight:700,letterSpacing:"-0.03em",color:"#1a1a1a"}}>TLG<span style={{color:"#185FA5",marginLeft:4}}>CRM</span></h1>
      <div style={{fontSize:13,color:"#8a8780",marginBottom:32}}>The Lifestyle Group</div>
      <div style={{fontSize:14,fontWeight:500,color:"#6b6960",marginBottom:12}}>Enter your 6-digit PIN</div>
      <input type="password" inputMode="numeric" maxLength={6} value={pin} onChange={function(e){var v=e.target.value.replace(/[^0-9]/g,"").slice(0,6);setPin(v);setError("");}} onKeyDown={function(e){if(e.key==="Enter"&&pin.length===6)handleSubmit();}} disabled={locked} autoFocus style={{width:"100%",padding:"14px 16px",fontSize:24,textAlign:"center",letterSpacing:"12px",borderRadius:10,border:"2px solid "+(error?"#E24B4A":"#e8e6df"),outline:"none",background:locked?"#f0eeea":"#fff",boxSizing:"border-box"}} />
      {error&&<div style={{fontSize:13,color:"#791F1F",marginTop:10,fontWeight:500}}>{error}{locked&&<span> ({Math.floor(lockTimer/60)}:{String(lockTimer%60).padStart(2,"0")})</span>}</div>}
      <button onClick={handleSubmit} disabled={pin.length!==6||locked} style={{width:"100%",padding:"12px",borderRadius:10,border:"none",background:pin.length===6&&!locked?"#185FA5":"#e8e6df",color:pin.length===6&&!locked?"#fff":"#b0ada6",fontSize:15,fontWeight:600,cursor:pin.length===6&&!locked?"pointer":"default",marginTop:16,transition:"all 0.15s"}}>Sign in</button>
    </div>
  </div>);
}
