...
async function sbInsert(table, data) {
  ...
  if (!r.ok) { var errText = await r.text(); console.error("INSERT", table, r.status, errText); alert("DB Error " + r.status + ": " + errText); return null; }
...
const ACTIVE_TEAM = ["Geoff Horen","Kalee Dunham","Brittney Schebler","Leesa","Lanie"];
  ...
  return (<div style={{ marginTop: 20 }}>
    ...
    {showAdd && <div style={{ background: "#f7f6f3", borderRadius: 10, padding: 16, marginTop: 10 }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
        ...
        <Field label="Created by" half><select style={inpS} value={form.created_by} onChange={set("created_by")}><option value="">Select...</option>{ACTIVE_TEAM.map(t => <option key={t}>{t}</option>)}</select></Field>
      </div>
...
function ActivityLog({ activities, projectId, contactId, onAdd, defaultAuthor }) {
  ...
  const doAdd = () => {
    ...
    if (atype === "Schedule") {
      ...
      if (schedTime) { var tp = schedTime.split(":"); var hr = parseInt(tp[0]); var mn = tp[1]; var ampm = hr >= 12 ? "PM" : "AM"; hr = hr % 12; if (hr === 0) hr = 12; schedText += " at " + hr + ":" + mn + " " + ampm; }
  ...
  return (<div>
    ...
    <div style={{background:"#f7f6f3",borderRadius:10,padding:14}}>
      <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap"}}>
        ...
        <select style={{...inpS,width:"auto",fontSize:13,padding:"6px 10px"}} value={author} onChange={e => setAuthor(e.target.value)}>{ACTIVE_TEAM.map(t => <option key={t}>{t}</option>)}</select>
      </div>
  ...
  return (<div>
    ...
    <div style={{background:"#fff",borderRadius:14,border:"1px solid #e8e6df",padding:"20px 24px",boxShadow:cardShadow}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:12}}>
        ...
        <Field label="Salesperson" half><select style={inpS} value={form.salesperson||""} onChange={set("salesperson")}><option value="">Select...</option>{ACTIVE_TEAM.map(s=><option key={s}>{s}</option>)}</select></Field>
        <Field label="Estimate $" half><DollarInput style={inpS} value={form.estimate_amount} onChange={function(v){setForm(function(f){return Object.assign({},f,{estimate_amount:v});});}} /></Field>
...
function Dashboard({ onOpenProject }) {
  ...
  const [allForecasted, setAllForecasted] = useState([]);
  ...
  useEffect(function() {
    sbGet("projects", `select=${DASH_COLS}&forecast_amount=not.is.null&stage=neq.Sold&stage=neq.Lost&limit=500`).then(function(r) {
      setAllForecasted(r || []);
      loadContacts(r);
    });
  }, []);
  ...
  useEffect(()=>{
    ...
    // Load carryover: sales booked this year whose lead came in a prior year (any prior year),
    // plus prior-year leads still active in the pipeline.
    if (year > 2000) {
      Promise.all([
        sbGet("projects", `select=${DASH_COLS}&stage=eq.Sold&date_sold=gte.${year}-01-01&date_sold=lte.${year}-12-31&limit=10000`),
        sbGet("projects", `select=${DASH_COLS}&lead_date=gte.${year-1}-01-01&lead_date=lte.${year-1}-12-31&stage=neq.Lost&stage=neq.Sold&limit=10000`)
      ]).then(([soldThisYear, priorActive]) => {
        const soldInCurrent = (soldThisYear||[]).filter(p => !(p.lead_date && p.lead_date >= year+"-01-01" && p.lead_date <= year+"-12-31"));
        const active = (priorActive||[]).filter(p=>p.stage!=="Sold"&&p.stage!=="Lost");
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:24}}>
    ...
    <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
      ...
      {metricCard("Sold",totalSold.toLocaleString()+(coSold>0?" ("+coSold+" carryover)":""),compData?compData.sold.length.toLocaleString():undefined)}
      ...
      {metricCard("Revenue",fmtC(totalRev),compData?fmtC(compData.rev):undefined)}
      {metricCard("Avg sale",fmtC(totalAvg),compData?fmtC(compData.avgSale):undefined)}
    ...
    {carryover&&<div style={{background:"#FAEEDA",borderRadius:10,padding:"12px 18px",borderLeft:"4px solid #EF9F27"}}>
      <div style={{fontSize:13,fontWeight:600,color:"#633806",marginBottom:4}}>Carryover into {year}</div>
      <div style={{fontSize:13,color:"#633806"}}>{carryover.active} prior-year lead{carryover.active===1?"":"s"} still active · {carryover.sold} sale{carryover.sold===1?"":"s"} closed in {year} from earlier leads{carryover.rev>0?" ("+fmtC(carryover.rev)+")":""} — included in Sold & Revenue above</div>
    </div>}
    ...
    {/* Forecasted Pipeline */}
    {(function() {
      var forecasted = allForecasted.filter(function(p) { return p.forecast_amount && p.stage !== "Sold" && p.stage !== "Lost"; });
...
function Reports({ onOpenProject, onOpenContact }) {
  ...
  var [funnelDrill, setFunnelDrill] = useState(null);

  function buildReportFunnel(filtered) {
    var all = filtered || [];
    var sold = all.filter(function(p){return p.stage==="Sold";});
    var lost = all.filter(function(p){return p.stage==="Lost";});
    var totalLeads = all.length;
    var discoveryDeaths = lost.filter(function(p){return p.stage_lost==="Discovery";}).length;
    var afterDiscovery = totalLeads - discoveryDeaths;
    var qualDeaths = lost.filter(function(p){return p.stage_lost==="Qualification";}).length;
    var firstVisitsInitiated = afterDiscovery - qualDeaths;
    var fvDeaths = lost.filter(function(p){return p.stage_lost==="First Visit";}).length;
    var firstVisitsCompleted = firstVisitsInitiated - fvDeaths;
    var prePresDeaths = lost.filter(function(p){return p.stage_lost==="Presentation"&&(p.lost_reason==="Cancel Prior"||p.lost_reason==="No Show");}).length;
    var presentations = firstVisitsCompleted - prePresDeaths;
    var presDeaths = lost.filter(function(p){return p.stage_lost==="Presentation"&&p.lost_reason!=="Cancel Prior"&&p.lost_reason!=="No Show";}).length;
    var atRevision = presentations - presDeaths;
    var revDeaths = lost.filter(function(p){return p.stage_lost==="Revision";}).length;
    var activeOpen = atRevision - revDeaths;
    var soldCount = sold.length;

    return [
      {label:"Total Leads",count:totalLeads,color:"#3974B7",projects:all},
      {label:"After Discovery",count:afterDiscovery,color:"#378ADD",deaths:discoveryDeaths,projects:all.filter(function(p){return p.stage!=="Lost"||p.stage_lost!=="Discovery";})},
      {label:"1st Visits Initiated",count:firstVisitsInitiated,color:"#85B7EB",deaths:qualDeaths,projects:all.filter(function(p){return p.stage!=="Lost"||(p.stage_lost!=="Discovery"&&p.stage_lost!=="Qualification");})},
      {label:"1st Visits Completed",count:firstVisitsCompleted,color:"#85B7EB",deaths:fvDeaths},
      {label:"Presentations",count:presentations,color:"#B5D4F4",deaths:prePresDeaths},
      {label:"Alive at Revision",count:atRevision,color:"#B5D4F4",deaths:presDeaths},
      {label:"Actively Open",count:activeOpen,color:"#E6F1FB",deaths:revDeaths},
      {label:"SOLD",count:soldCount,color:"#2D6A4F",projects:sold}
    ];
  }
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:20}}>
    ...
    {results&&<div>
      ...
      {/* Sales funnel for filtered results */}
      {(function() {
        var funnel = buildReportFunnel(results.filtered);
        var maxCount = Math.max.apply(null, funnel.map(function(g){return g.count;}));
        return <div style={{background:"#fff",border:"1px solid #e8e6df",borderRadius:10,padding:16,marginBottom:20}}>
          <div style={{fontSize:14,fontWeight:600,marginBottom:14,color:"#6b6960"}}>Sales funnel — filtered results</div>
          {funnel.map(function(gate, i) {
            var w = maxCount > 0 ? Math.max((gate.count / maxCount) * 100, 6) : 0;
            var pct = funnel[0].count > 0 ? Math.round((gate.count / funnel[0].count) * 100) : 0;
            var isSold = gate.label === "SOLD";
            return <div key={i} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
              <span style={{fontSize:11,minWidth:110,color:"#6b6960",textAlign:"right",fontWeight:500}}>{gate.label}</span>
              <div style={{flex:1,height:26,background:"#f0eeea",borderRadius:6,overflow:"hidden",position:"relative"}}>
                <div style={{width:w+"%",height:"100%",background:isSold?"#2D6A4F":gate.color,borderRadius:6,display:"flex",alignItems:"center",paddingLeft:8,fontSize:12,fontWeight:600,color:isSold||gate.color==="#3974B7"||gate.color==="#378ADD"?"#fff":"#243F81",minWidth:40}}>{gate.count}</div>
              </div>
              {gate.deaths>0&&<span style={{fontSize:10,color:"#791F1F",fontWeight:600,minWidth:30}}>-{gate.deaths}</span>}
              <span style={{fontSize:11,color:"#8a8780",minWidth:35,textAlign:"right"}}>{pct}%</span>
            </div>;
          })}
          {funnel[0].count > 0 && <div style={{display:"flex",gap:16,flexWrap:"wrap",marginTop:12,fontSize:12,color:"#6b6960"}}>
            <span>Close rate: <b>{Math.round((funnel[7].count / funnel[0].count) * 100)}%</b></span>
            {funnel[3].count > 0 && <span>Sold from 1st visits: <b>{Math.round((funnel[7].count / funnel[3].count) * 100)}%</b></span>}
            {funnel[4].count > 0 && <span>Sold from presentations: <b>{Math.round((funnel[7].count / funnel[4].count) * 100)}%</b></span>}
          </div>}
        </div>;
      })()}
...
function Scorecard({ onOpenProject }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      soldY.forEach(p => { if (p.contact_id) cids[p.contact_id] = true; });
      const idList = Object.keys(cids);
      if (idList.length) {
        const batches = [];
...
function ManagementConsole({ onOpenProject }) {
  ...
  useEffect(function() {
    Promise.all([
      ...
      // Sold & revenue per rep count by close date (date_sold this year), not lead cohort
      soldY.forEach(function(p) {
        var sp = p.salesperson || "Unassigned";
        if (!spMap[sp]) spMap[sp] = { name: sp, leads: 0, sold: 0, rev: 0, est: 0, pipelineVal: 0, pipelineCount: 0, overdueCount: 0, warningCount: 0, daysToClose: [], lost: 0 };
        spMap[sp].sold++;
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:16}}>
    ...
    {section==="mchome"&&<div style={{display:"grid",gridTemplateColumns:"repeat(3, 1fr)",gap:10,maxWidth:480}}>
      {[
        ...
        {id:"stale",label:"Stale Lead Analysis",icon:function(){return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#243F81" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>;}},
...
function GoalThermometers() {
  ...
  useEffect(function() {
    Promise.all([
      ...
      // Build monthly actuals for current year (leads by lead_date, sold by close date)
      ...
      // Prior year same-period totals (leads by lead_date, sold by close date)
...
function OrderDashboard({ onOpenProject }) {
  ...
  var [filterPhase, setFilterPhase] = useState("");
  ...
  var statusColor = function(s) {
    ...
    if (s==="Shipped") return {bg:"#FAEEDA",color:"#633806"};
    if (s==="Back-Ordered") return {bg:"#FAEEDA",color:"#633806"};
    if (s==="Received") return {bg:"#EAF3DE",color:"#27500A"};
    if (s==="Installed") return {bg:"#EAF3DE",color:"#173404"};
    if (s==="Issue") return {bg:"#FCEBEB",color:"#791F1F"};
    return {bg:"#f0eeea",color:"#6b6960"};
  };

  useEffect(function() {
    Promise.all([
      sbGet("project_orders", "select=*&order=projected_arrival.asc.nullslast,order_date.desc&limit=1000"),
      sbGet("projects", "select=id,job_name,contact_id,stage&stage=neq.Lost&limit=5000")
    ]).then(function(results) {
      var orders = results[0] || [];
      var projs = results[1] || [];
      var pMap = {}; projs.forEach(function(p) { pMap[p.id] = p; });
      setProjects(pMap);
      setItems(orders);
      ...
      var idList = Object.keys(cids);
      if (idList.length > 0) {
        var batches = []; for (var i=0; i<idList.length; i+=200) batches.push(idList.slice(i,i+200));
        Promise.all(batches.map(function(b) { return sbGetCached("contacts", "id=in.(" + b.join(",") + ")&select=id,first_name,last_name"); })).then(function(res) {
          var cmap = {}; res.forEach(function(b) { (b||[]).forEach(function(c) { cmap[c.id] = c; }); });
          setContacts(cmap);
        });
      }
      setLoading(false);
  ...
  if (sortBy === "arrival") {
    filtered.sort(function(a,b) {
      var ad = a.projected_arrival || "9999"; var bd = b.projected_arrival || "9999";
      return ad.localeCompare(bd);
    });
  } else if (sortBy === "status") {
    var statusOrder = {"Issue":0,"Back-Ordered":1,"Ordered":2,"Shipped":3,"Received":4,"Installed":5};
    filtered.sort(function(a,b) { return (statusOrder[a.status]||9) - (statusOrder[b.status]||9); });
  } else if (sortBy === "project") {
    filtered.sort(function(a,b) {
      var pa = projects[a.project_id]; var pb = projects[b.project_id];
      return ((pa?pa.job_name:"")||"").localeCompare((pb?pb.job_name:"")||"");
    });
  } else if (sortBy === "vendor") {
    filtered.sort(function(a,b) { return ((a.vendor||"").localeCompare(b.vendor||"")); });
  }

  var pending = items.filter(function(i) { return i.status==="Ordered"||i.status==="Shipped"||i.status==="Back-Ordered"; });
  var pastDue = pending.filter(function(i) { return i.projected_arrival && i.projected_arrival < today; });
  var issues = items.filter(function(i) { return i.status==="Issue"; });
  var totalPending = pending.reduce(function(s,i) { return s + ((parseFloat(i.unit_price)||0)*(i.quantity||1)); }, 0);

  var vendors = {}; items.forEach(function(i) { if (i.vendor) vendors[i.vendor] = true; });
  var vendorList = Object.keys(vendors).sort();

  if (loading) return <div style={{padding:40,textAlign:"center",color:"#8a8780"}}>Loading orders...</div>;

  return (<div>
    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:16}}>
      <div style={{background:"#fff",borderRadius:8,border:"0.5px solid #e8e6df",padding:"10px 14px",flex:"1 1 80px",minWidth:80}}>
        <div style={{fontSize:11,color:"#8a8780"}}>Total items</div>
        <div style={{fontSize:20,fontWeight:500}}>{items.length}</div>
      </div>
      <div style={{background:"#fff",borderRadius:8,border:"0.5px solid #e8e6df",padding:"10px 14px",flex:"1 1 80px",minWidth:80}}>
        <div style={{fontSize:11,color:"#8a8780"}}>Pending</div>
        <div style={{fontSize:20,fontWeight:500,color:"#BA7517"}}>{pending.length}</div>
      </div>
      <div style={{background:pastDue.length>0?"#FCEBEB":"#fff",borderRadius:8,border:pastDue.length>0?"0.5px solid #F09595":"0.5px solid #e8e6df",padding:"10px 14px",flex:"1 1 80px",minWidth:80}}>
        <div style={{fontSize:11,color:pastDue.length>0?"#791F1F":"#8a8780"}}>Past due</div>
        <div style={{fontSize:20,fontWeight:500,color:pastDue.length>0?"#791F1F":"#1a1a1a"}}>{pastDue.length}</div>
      </div>
      <div style={{background:issues.length>0?"#FCEBEB":"#fff",borderRadius:8,border:issues.length>0?"0.5px solid #F09595":"0.5px solid #e8e6df",padding:"10px 14px",flex:"1 1 80px",minWidth:80}}>
        <div style={{fontSize:11,color:issues.length>0?"#791F1F":"#8a8780"}}>Issues</div>
        <div style={{fontSize:20,fontWeight:500,color:issues.length>0?"#791F1F":"#1a1a1a"}}>{issues.length}</div>
      </div>
    </div>

    {totalPending>0&&<div style={{fontSize:12,color:"#6b6960",marginBottom:12}}>Pending materials value: <b>{fmtC(totalPending)}</b></div>}

    <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
      <select style={{...inpS,width:"auto",fontSize:12}} value={filterStatus} onChange={function(e){setFilterStatus(e.target.value);}}><option value="">All statuses</option>{STATUSES.map(function(s){return <option key={s}>{s}</option>;})}</select>
      <select style={{...inpS,width:"auto",fontSize:12}} value={filterPhase} onChange={function(e){setFilterPhase(e.target.value);}}><option value="">All phases</option>{PHASES.map(function(p){return <option key={p}>{p}</option>;})}</select>
      <select style={{...inpS,width:"auto",fontSize:12}} value={filterVendor} onChange={function(e){setFilterVendor(e.target.value);}}><option value="">All vendors</option>{vendorList.map(function(v){return <option key={v}>{v}</option>;})}</select>
      <select style={{...inpS,width:"auto",fontSize:12}} value={sortBy} onChange={function(e){setSortBy(e.target.value);}}><option value="arrival">Sort: arrival date</option><option value="status">Sort: status</option><option value="project">Sort: project</option><option value="vendor">Sort: vendor</option></select>
    </div>

    {filtered.length===0&&<div style={{padding:24,textAlign:"center",color:"#8a8780",fontSize:13}}>No items match your filters.</div>}

    {filtered.map(function(item) {
      var sc = statusColor(item.status);
      var proj = projects[item.project_id];
      var projName = proj ? proj.job_name : "Unknown project";
      var ct = proj && proj.contact_id ? contacts[proj.contact_id] : null;
      var cName = ct ? (ct.first_name||"") + " " + (ct.last_name||"") : "";
      var lineTotal = (parseFloat(item.unit_price)||0) * (item.quantity||1);
      var isPastDue = item.projected_arrival && item.projected_arrival < today && (item.status==="Ordered"||item.status==="Shipped"||item.status==="Back-Ordered");
      var daysLate = isPastDue ? Math.round((now - new Date(item.projected_arrival+"T00:00:00")) / 86400000) : 0;

      return <div key={item.id} style={{border:isPastDue?"0.5px solid #F09595":item.status==="Issue"?"0.5px solid #F09595":"0.5px solid #e8e6df",borderRadius:8,padding:"10px 14px",marginBottom:6,background:isPastDue?"#FCEBEB22":"transparent",cursor:"pointer"}} onClick={function(){if(proj)onOpenProject(proj);}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:13,fontWeight:600}}>{item.item_name}{item.quantity>1?" (×"+item.quantity+")":""}</div>
            <div style={{fontSize:11,color:"#185FA5",fontWeight:500,marginTop:1}}>{projName}{cName.trim()?" — "+cName.trim():""}</div>
            <div style={{fontSize:11,color:"#8a8780",marginTop:1}}>
              {item.vendor&&<span>{item.vendor}</span>}
              {item.vendor&&item.phase&&<span> · </span>}
              {item.phase&&<span>{item.phase}</span>}
              {lineTotal>0&&<span> · {fmtC(lineTotal)}</span>}
            </div>
          </div>
          <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:3,flexShrink:0}}>
            <span style={{fontSize:10,padding:"2px 8px",borderRadius:4,background:sc.bg,color:sc.color,fontWeight:600}}>{item.status}</span>
            {item.projected_arrival&&<span style={{fontSize:10,color:isPastDue?"#791F1F":"#8a8780",fontWeight:isPastDue?600:400}}>{isPastDue?daysLate+"d late":"Due "+fmtD(item.projected_arrival)}</span>}
          </div>
        </div>
        {item.damage_notes&&<div style={{fontSize:11,color:"#791F1F",fontWeight:500,marginTop:3}}>{item.damage_notes}</div>}
      </div>;
    })}
  </div>);
}
...
/* ── Order Tracker (within Project Detail) ── */
function OrderTracker({ projectId, contactId }) {
  var [items, setItems] = useState([]);
  var [loading, setLoading] = useState(true);
  ...
  function loadItems() {
    sbGet("project_orders", "select=*&project_id=eq." + projectId + "&order=order_date.desc.nullslast,created_at.desc&limit=200").then(function(r) {
      setItems(r || []);
      setLoading(false);
    });
  }
  ...
  var emptyForm = {item_name:"",phase:"",vendor:"",po_number:"",model_number:"",quantity:1,unit_price:"",ordered_by:"",order_date:"",projected_arrival:"",tracking_number:"",status:"Ordered",received_date:"",received_by:"",damage_notes:"",warehouse_location:"",notes:""};

  function openNew() { setForm(Object.assign({}, emptyForm)); setShowNew(true); setDetail(null); setEditMode(false); }
  function openDetail(item) { setDetail(item); setEditMode(false); setShowNew(false); setForm(Object.assign({}, emptyForm, item, {unit_price: item.unit_price || "", quantity: item.quantity || 1})); }
  ...
  function saveItem() {
    ...
    data.unit_price = data.unit_price ? parseFloat(data.unit_price) : null;
    data.quantity = data.quantity ? parseInt(data.quantity) : 1;
    data.order_date = data.order_date || null;
    data.projected_arrival = data.projected_arrival || null;
    ...
    data.model_number = data.model_number || null;
    data.vendor = data.vendor || null;
    data.ordered_by = data.ordered_by || null;
    data.received_by = data.received_by || null;
    data.damage_notes = data.damage_notes || null;
    data.warehouse_location = data.warehouse_location || null;
    ...
    if (detail && detail.id) {
      sbUpdate("project_orders", detail.id, data).then(function(r) {
        setSaving(false);
        if (!r) { alert("Error saving — check that the project_orders table exists in Supabase"); return; }
        setEditMode(false); loadItems();
        setDetail(Object.assign({}, detail, data));
      });
    } else {
      sbInsert("project_orders", data).then(function(r) {
        setSaving(false);
        if (!r) { alert("Error saving — check that the project_orders table exists in Supabase"); return; }
        setShowNew(false); loadItems();
        openDetail(r);
      });
    }
  }

  var totalCost = items.reduce(function(s, i) { return s + ((parseFloat(i.unit_price)||0) * (i.quantity||1)); }, 0);
  var received = items.filter(function(i) { return i.status==="Received"||i.status==="Installed"; }).length;
  var pending = items.filter(function(i) { return i.status==="Ordered"||i.status==="Shipped"||i.status==="Back-Ordered"; }).length;
  var issues = items.filter(function(i) { return i.status==="Issue"; }).length;

  var renderForm = function() {
    return <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={{fontSize:12,fontWeight:700,color:"#8a8780",textTransform:"uppercase",letterSpacing:"0.06em"}}>Item details</div>
      <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
        <Field label="Item name *" half><input style={inpS} value={form.item_name||""} onChange={set("item_name")} placeholder="e.g., Crystal cabinet — upper 36W"/></Field>
        <Field label="Model #" half><input style={inpS} value={form.model_number||""} onChange={set("model_number")}/></Field>
        <Field label="Quantity" half><input type="number" style={inpS} value={form.quantity||1} onChange={set("quantity")} min="1"/></Field>
        <Field label="Unit price (before tax)" half><DollarInput style={inpS} value={form.unit_price} onChange={function(v){setForm(function(f){return Object.assign({},f,{unit_price:v});});}}/></Field>
        <Field label="Phase" half><select style={inpS} value={form.phase||""} onChange={set("phase")}><option value="">Select...</option>{PHASES.map(function(p){return <option key={p}>{p}</option>;})}</select></Field>
        <Field label="Vendor" half><input style={inpS} value={form.vendor||""} onChange={set("vendor")} placeholder="e.g., Ferguson, Crystal Cabinets"/></Field>
        <Field label="PO / Order #" half><input style={inpS} value={form.po_number||""} onChange={set("po_number")}/></Field>
        <Field label="Tracking #" half><input style={inpS} value={form.tracking_number||""} onChange={set("tracking_number")}/></Field>
      </div>

      <div style={{fontSize:12,fontWeight:700,color:"#8a8780",textTransform:"uppercase",letterSpacing:"0.06em"}}>Ordering</div>
      <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
        <Field label="Ordered by" half><select style={inpS} value={form.ordered_by||""} onChange={set("ordered_by")}><option value="">Select...</option>{ACTIVE_TEAM.map(function(t){return <option key={t}>{t}</option>;})}</select></Field>
        <Field label="Date ordered" half><input type="date" style={inpS} value={form.order_date||""} onChange={set("order_date")}/></Field>
        <Field label="Projected arrival" half><input type="date" style={inpS} value={form.projected_arrival||""} onChange={set("projected_arrival")}/></Field>
        <Field label="Status" half><select style={inpS} value={form.status||"Ordered"} onChange={set("status")}>{STATUSES.map(function(s){return <option key={s}>{s}</option>;})}</select></Field>
      </div>

      <div style={{fontSize:12,fontWeight:700,color:"#8a8780",textTransform:"uppercase",letterSpacing:"0.06em"}}>Receiving</div>
      <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
        <Field label="Date received" half><input type="date" style={inpS} value={form.received_date||""} onChange={set("received_date")}/></Field>
        <Field label="Received by" half><select style={inpS} value={form.received_by||""} onChange={set("received_by")}><option value="">Select...</option>{ACTIVE_TEAM.map(function(t){return <option key={t}>{t}</option>;})}</select></Field>
        <Field label="Warehouse location" half><input style={inpS} value={form.warehouse_location||""} onChange={set("warehouse_location")} placeholder="e.g., Rack B-3"/></Field>
        <Field label="Damage notes" half><input style={inpS} value={form.damage_notes||""} onChange={set("damage_notes")} placeholder="Describe any damage"/></Field>
      </div>

      <Field label="Notes"><textarea style={{...inpS,minHeight:50,resize:"vertical"}} value={form.notes||""} onChange={set("notes")}/></Field>

      <div style={{display:"flex",justifyContent:"flex-end",gap:8}}>
        <button onClick={function(){if(showNew){setShowNew(false);}else{setEditMode(false);}}} style={btnSec}>Cancel</button>
        <button onClick={saveItem} disabled={!form.item_name||saving} style={{...btnP,opacity:form.item_name?1:0.5}}>{saving?"Saving...":(detail?"Save item":"Save item")}</button>
      </div>
    </div>;
  };
  ...
  var renderDetail = function() {
    ...
    return <div style={{display:"flex",flexDirection:"column",gap:10}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:8}}>
        <div>
          <div style={{fontSize:16,fontWeight:700}}>{item.item_name}</div>
          {item.phase&&<div style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:"#E6F1FB",color:"#0C447C",fontWeight:500,display:"inline-block",marginTop:4}}>{item.phase}</div>}
        </div>
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          <span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:sc.bg,color:sc.color,fontWeight:600}}>{item.status}</span>
          <button onClick={function(){setEditMode(true);}} style={btnSec}>Edit</button>
        </div>
      </div>

      <div style={{display:"flex",flexWrap:"wrap",gap:16,fontSize:13}}>
        {item.model_number&&<div><span style={{color:"#8a8780",fontSize:11,fontWeight:600}}>Model</span><br/>{item.model_number}</div>}
        {item.vendor&&<div><span style={{color:"#8a8780",fontSize:11,fontWeight:600}}>Vendor</span><br/>{item.vendor}</div>}
        {item.quantity>1&&<div><span style={{color:"#8a8780",fontSize:11,fontWeight:600}}>Qty</span><br/>{item.quantity}</div>}
        {item.unit_price&&<div><span style={{color:"#8a8780",fontSize:11,fontWeight:600}}>Unit price</span><br/>{fmtC(item.unit_price)}</div>}
        {lineTotal>0&&item.quantity>1&&<div><span style={{color:"#8a8780",fontSize:11,fontWeight:600}}>Line total</span><br/><b>{fmtC(lineTotal)}</b></div>}
        {item.po_number&&<div><span style={{color:"#8a8780",fontSize:11,fontWeight:600}}>PO #</span><br/>{item.po_number}</div>}
        {item.tracking_number&&<div><span style={{color:"#8a8780",fontSize:11,fontWeight:600}}>Tracking</span><br/>{item.tracking_number}</div>}
      </div>

      <div style={{display:"flex",flexWrap:"wrap",gap:16,fontSize:13}}>
        {item.ordered_by&&<div><span style={{color:"#8a8780",fontSize:11,fontWeight:600}}>Ordered by</span><br/>{item.ordered_by}</div>}
        {item.order_date&&<div><span style={{color:"#8a8780",fontSize:11,fontWeight:600}}>Ordered</span><br/>{fmtD(item.order_date)}</div>}
        {item.projected_arrival&&<div><span style={{color:"#8a8780",fontSize:11,fontWeight:600}}>Est. arrival</span><br/>{fmtD(item.projected_arrival)}</div>}
      </div>

      {(item.received_date||item.received_by||item.warehouse_location||item.damage_notes)&&<div style={{background:item.damage_notes?"#FCEBEB44":"#f7f6f3",borderRadius:8,padding:"10px 14px",border:item.damage_notes?"0.5px solid #F09595":"none"}}>
        <div style={{fontSize:11,fontWeight:700,color:item.damage_notes?"#791F1F":"#8a8780",textTransform:"uppercase",letterSpacing:"0.04em",marginBottom:4}}>{item.damage_notes?"Receiving — issue flagged":"Receiving"}</div>
        <div style={{display:"flex",flexWrap:"wrap",gap:16,fontSize:13}}>
          {item.received_date&&<div><span style={{color:"#8a8780",fontSize:11}}>Arrived</span><br/>{fmtD(item.received_date)}</div>}
          {item.received_by&&<div><span style={{color:"#8a8780",fontSize:11}}>Received by</span><br/>{item.received_by}</div>}
          {item.warehouse_location&&<div><span style={{color:"#8a8780",fontSize:11}}>Location</span><br/>{item.warehouse_location}</div>}
        </div>
        {item.damage_notes&&<div style={{color:"#791F1F",fontWeight:500,fontSize:12,marginTop:4}}>{item.damage_notes}</div>}
      </div>}

      {item.notes&&<div style={{fontSize:12,color:"#6b6960",fontStyle:"italic",background:"#f7f6f3",borderRadius:6,padding:"8px 10px"}}>{item.notes}</div>}
    </div>;
  };
  ...
  return (<div style={{marginTop:20}}>
    <div onClick={function(){if(!detail&&!showNew)setExpanded(!expanded);}} style={{display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer",padding:"10px 0",borderBottom:"1px solid #e8e6df"}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <span style={{fontSize:14,fontWeight:600,color:"#243F81"}}>Materials & orders</span>
        <span style={{fontSize:12,color:"#8a8780"}}>({items.length} items{totalCost>0?" · "+fmtC(totalCost):""})</span>
        {issues>0&&<span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:"#FCEBEB",color:"#791F1F",fontWeight:600}}>{issues} issue{issues>1?"s":""}</span>}
        {pending>0&&<span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:"#FAEEDA",color:"#633806",fontWeight:600}}>{pending} pending</span>}
      </div>
      <span style={{fontSize:12,color:"#8a8780"}}>{expanded||detail||showNew?"▾":"▸"}</span>
    </div>

    {(expanded||detail||showNew)&&<div style={{paddingTop:12}}>
      {!detail&&!showNew&&<>
        <div style={{display:"flex",justifyContent:"flex-end",marginBottom:10}}>
          <button onClick={openNew} style={btnP}>+ Add item</button>
        </div>

        {items.length===0&&<div style={{padding:16,textAlign:"center",color:"#8a8780",fontSize:13}}>No materials ordered yet.</div>}

        {items.map(function(item) {
          var sc = statusColor(item.status);
          var lineTotal = (parseFloat(item.unit_price)||0) * (item.quantity||1);
          return <div key={item.id} onClick={function(){openDetail(item);}} style={{border:item.status==="Issue"?"0.5px solid #F09595":"0.5px solid #e8e6df",borderRadius:8,padding:"10px 14px",marginBottom:6,cursor:"pointer",transition:"all 0.15s"}} onMouseEnter={function(e){e.currentTarget.style.borderColor="#185FA5";}} onMouseLeave={function(e){e.currentTarget.style.borderColor=item.status==="Issue"?"#F09595":"#e8e6df";}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              ...
              <span style={{fontSize:10,padding:"2px 8px",borderRadius:4,background:sc.bg,color:sc.color,fontWeight:600,flexShrink:0}}>{item.status}</span>
            </div>
            {item.damage_notes&&<div style={{fontSize:11,color:"#791F1F",fontWeight:500,marginTop:3}}>{item.damage_notes}</div>}
          </div>;
        })}
      ...
      {showNew&&renderForm()}
      {detail&&!editMode&&<div>
        <button onClick={function(){setDetail(null);}} style={{background:"none",border:"none",cursor:"pointer",fontSize:12,color:"#185FA5",fontWeight:600,padding:0,marginBottom:10}}>&#8592; Back to items</button>
        {renderDetail()}
      </div>}
      {detail&&editMode&&<div>
        <button onClick={function(){setEditMode(false);}} style={{background:"none",border:"none",cursor:"pointer",fontSize:12,color:"#185FA5",fontWeight:600,padding:0,marginBottom:10}}>&#8592; Cancel edit</button>
        {renderForm()}
      </div>}
    </div>}
  </div>);
...
/* ── SOP Viewer (Management Console) ── */
function SOPViewer() {
  var [sops, setSops] = useState([]);
  var [loading, setLoading] = useState(true);
  var [activeCat, setActiveCat] = useState(null);
  var [activeSop, setActiveSop] = useState(null);
  var [searchQ, setSearchQ] = useState("");

  var CATS = [
    {id:"Daily",icon:"☀️",desc:"Start-of-day and end-of-day tasks"},
    {id:"Weekly",icon:"📅",desc:"Monday and Friday recurring items"},
    {id:"Bi-Weekly",icon:"💰",desc:"Payroll and bi-weekly processes"},
    {id:"Quarterly",icon:"📊",desc:"Quarterly reviews and updates"},
    {id:"Annual",icon:"🗓️",desc:"Yearly planning and renewals"},
    {id:"Event-Driven",icon:"⚡",desc:"Triggered by specific business events"},
    {id:"Operational",icon:"🔧",desc:"Vendor ordering and purchasing"},
    {id:"Reference",icon:"📋",desc:"Gap analysis and reference docs"}
  ];

  useEffect(function() {
    sbGet("sops", "select=id,sop_id,title,category,body&order=sop_id.asc&limit=200").then(function(r) {
      setSops(r || []);
      setLoading(false);
    });
  }, []);
  ...
  if (searchQ) {
    ...
    filtered = sops.filter(function(s) {
      return (s.title||"").toLowerCase().indexOf(q) >= 0 || (s.sop_id||"").toLowerCase().indexOf(q) >= 0 || (s.body||"").replace(/<[^>]*>/g,"").toLowerCase().indexOf(q) >= 0;
    });
  }
  ...
  if (loading) return <div style={{padding:20,textAlign:"center",color:"#8a8780"}}>Loading SOPs...</div>;

  // Viewing a specific SOP
  if (activeSop) {
    return <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <button onClick={function(){setActiveSop(null);}} style={{background:"none",border:"none",cursor:"pointer",fontSize:13,color:"#185FA5",fontWeight:600}}>&#8592; Back to {activeCat || "SOPs"}</button>
      </div>
      <div style={{background:"#fff",border:"1px solid #e8e6df",borderRadius:12,padding:"20px 24px"}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:4}}>
          <span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:"#E6F1FB",color:"#0C447C",fontWeight:600}}>{activeSop.sop_id}</span>
          <span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:"#f0eeea",color:"#6b6960"}}>{activeSop.category}</span>
        </div>
        <div style={{fontSize:18,fontWeight:700,marginBottom:16,color:"#1a1a1a"}}>{activeSop.title}</div>
        <style dangerouslySetInnerHTML={{__html:".sop-body table{width:100%;border-collapse:collapse;margin:12px 0;font-size:13px}.sop-body td,.sop-body th{padding:8px 12px;border:1px solid #e8e6df;vertical-align:top;line-height:1.5}.sop-body tr:nth-child(even){background:#f7f6f3}.sop-body strong{color:#243F81}.sop-body ol,.sop-body ul{padding-left:24px;margin:8px 0}.sop-body li{margin-bottom:4px;line-height:1.6}.sop-body p{margin:8px 0;line-height:1.6}.sop-body h1,.sop-body h2,.sop-body h3{color:#243F81;margin:16px 0 8px}.sop-body td:first-child{font-weight:600;white-space:nowrap;background:#f7f6f3;min-width:100px}"}}/>
        <div className="sop-body" style={{fontSize:14,color:"#1a1a1a"}} dangerouslySetInnerHTML={{__html:activeSop.body}}/>
      </div>
    </div>;
  }

  // Category view - list SOPs in selected category
  if (activeCat) {
    var catSops = byCat[activeCat] || [];
    return <div style={{display:"flex",flexDirection:"column",gap:12}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <button onClick={function(){setActiveCat(null);}} style={{background:"none",border:"none",cursor:"pointer",fontSize:13,color:"#185FA5",fontWeight:600}}>&#8592; All categories</button>
        <span style={{fontSize:16,fontWeight:600}}>{activeCat} SOPs</span>
        <span style={{fontSize:12,color:"#8a8780"}}>({catSops.length})</span>
      </div>
      {catSops.length===0&&<div style={{padding:20,textAlign:"center",color:"#8a8780"}}>No SOPs in this category yet.</div>}
      {catSops.map(function(sop) {
        return <div key={sop.id} onClick={function(){setActiveSop(sop);}} style={{background:"#fff",border:"1px solid #e8e6df",borderRadius:10,padding:"14px 16px",cursor:"pointer",transition:"all 0.15s"}} onMouseEnter={function(e){e.currentTarget.style.borderColor="#185FA5";}} onMouseLeave={function(e){e.currentTarget.style.borderColor="#e8e6df";}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:"#E6F1FB",color:"#0C447C",fontWeight:600,flexShrink:0}}>{sop.sop_id}</span>
            <span style={{fontSize:14,fontWeight:600}}>{sop.title}</span>
          </div>
          <div style={{fontSize:12,color:"#8a8780",marginTop:4,paddingLeft:0}}>{(sop.body||"").replace(/<[^>]*>/g,"").slice(0,120)}...</div>
        </div>;
      })}
    </div>;
  }

  // Main view - category grid + search
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
      <span style={{fontSize:16,fontWeight:600}}>Standard Operating Procedures</span>
      <span style={{fontSize:12,color:"#8a8780"}}>{sops.length} SOPs across {Object.keys(byCat).length} categories</span>
    </div>

    <input style={{...inpS,fontSize:13,padding:"10px 14px"}} placeholder="Search all SOPs by title or content..." value={searchQ} onChange={function(e){setSearchQ(e.target.value);}}/>

    {searchQ ? <>
      <div style={{fontSize:13,color:"#6b6960",fontWeight:600}}>{filtered.length} results for "{searchQ}"</div>
      {filtered.map(function(sop) {
        return <div key={sop.id} onClick={function(){setActiveSop(sop);}} style={{background:"#fff",border:"1px solid #e8e6df",borderRadius:10,padding:"12px 16px",cursor:"pointer",transition:"all 0.15s"}} onMouseEnter={function(e){e.currentTarget.style.borderColor="#185FA5";}} onMouseLeave={function(e){e.currentTarget.style.borderColor="#e8e6df";}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:"#E6F1FB",color:"#0C447C",fontWeight:600}}>{sop.sop_id}</span>
            <span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:"#f0eeea",color:"#6b6960"}}>{sop.category}</span>
            <span style={{fontSize:14,fontWeight:600}}>{sop.title}</span>
          </div>
        </div>;
      })}
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(180px, 1fr))",gap:10}}>
        {CATS.map(function(cat) {
          var count = (byCat[cat.id] || []).length;
          return <div key={cat.id} onClick={function(){setActiveCat(cat.id);}} style={{background:"#fff",borderRadius:12,padding:"16px",cursor:"pointer",border:"1px solid #e8e6df",transition:"all 0.15s"}} onMouseEnter={function(e){e.currentTarget.style.borderColor="#185FA5";e.currentTarget.style.boxShadow="0 2px 8px rgba(36,63,129,0.1)";}} onMouseLeave={function(e){e.currentTarget.style.borderColor="#e8e6df";e.currentTarget.style.boxShadow="none";}}>
            <div style={{fontSize:24,marginBottom:8}}>{cat.icon}</div>
            <div style={{fontSize:14,fontWeight:600,color:"#243F81"}}>{cat.id}</div>
            <div style={{fontSize:11,color:"#8a8780",marginTop:2}}>{cat.desc}</div>
            <div style={{fontSize:12,fontWeight:600,color:"#185FA5",marginTop:8}}>{count} SOP{count!==1?"s":""}</div>
          </div>;
        })}
      </div>
    </>}
...
/* ── Password Vault (Hidden Console) ── */
function PasswordVault({ authUser }) {
  ...
  var [showPw, setShowPw] = useState({});

  var VAULT_CATS = ["Banking & Finance","Insurance","Software & Apps","Vendors & Suppliers","Utilities","Social Media","Website & Marketing","Government & Tax","Communication","Office & Equipment","Other"];
  var ACCESS_LEVELS = ["Owner","Admin","Sales","All"];

  function loadEntries() {
    sbGet("vault", "select=*&order=category.asc,account_name.asc&limit=500").then(function(r) {
      setEntries(r || []);
      setLoading(false);
    });
  }
  ...
  var userRole = authUser ? authUser.role : "Sales";
  var visible = entries.filter(function(e) {
    if (e.access_level === "All") return true;
    if (e.access_level === "Sales") return true;
    if (e.access_level === "Admin" && (userRole === "Admin" || userRole === "Owner")) return true;
    if (e.access_level === "Owner" && userRole === "Owner") return true;
    return false;
  });

  var set = function(k) { return function(e) { setForm(function(f) { return Object.assign({}, f, {[k]: e.target.value}); }); }; };

  var emptyForm = {account_name:"",category:"",url:"",username:"",email:"",password:"",pin:"",security_questions:"",account_number:"",phone:"",contact_name:"",access_level:"Admin",assigned_to:"",notes:"",active:true};

  function openNew() { setForm(Object.assign({}, emptyForm)); setShowNew(true); setDetail(null); setEditMode(false); setShowPw({}); }
  function openDetail(entry) { setDetail(entry); setEditMode(false); setShowNew(false); setForm(Object.assign({}, emptyForm, entry)); setShowPw({}); }
  ...
  function saveEntry() {
    ...
    data.security_questions = data.security_questions || null;
    ...
    if (detail && detail.id) {
      sbUpdate("vault", detail.id, data).then(function() {
        setSaving(false); setEditMode(false);
        loadEntries();
        setDetail(Object.assign({}, detail, data));
      });
    } else {
      sbInsert("vault", data).then(function(r) {
  ...
  function togglePw(field) { setShowPw(function(p) { var n = Object.assign({}, p); n[field] = !n[field]; return n; }); }

  function maskedField(label, value, field) {
    if (!value) return null;
    var isVisible = showPw[field];
    return <div style={{display:"flex",alignItems:"center",gap:8}}>
      <div>
        <span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>{label}</span><br/>
        <span style={{fontFamily:isVisible?"inherit":"monospace",fontSize:14,letterSpacing:isVisible?0:"0.1em"}}>{isVisible ? value : "••••••••"}</span>
      </div>
      <button onClick={function(){togglePw(field);}} style={{padding:"3px 8px",borderRadius:4,border:"1px solid #d0cec7",background:"#fff",fontSize:10,cursor:"pointer",color:"#6b6960",marginTop:12}}>{isVisible?"Hide":"Show"}</button>
      <button onClick={function(){navigator.clipboard.writeText(value).then(function(){});}} style={{padding:"3px 8px",borderRadius:4,border:"1px solid #d0cec7",background:"#fff",fontSize:10,cursor:"pointer",color:"#185FA5",marginTop:12}}>Copy</button>
    </div>;
  }
  ...
  if (searchQ) {
    ...
    filtered = filtered.filter(function(e) {
      return [e.account_name,e.category,e.username,e.email,e.url,e.notes,e.assigned_to].filter(Boolean).join(" ").toLowerCase().indexOf(q) >= 0;
    });
  }
  ...
  var accessColor = function(level) {
    if (level === "Owner") return {bg:"#FEF0F0",color:"#791F1F"};
    if (level === "Admin") return {bg:"#FAEEDA",color:"#633806"};
    if (level === "Sales") return {bg:"#E6F1FB",color:"#0C447C"};
    return {bg:"#f0eeea",color:"#6b6960"};
  };

  var renderForm = function() {
    return <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{fontSize:12,fontWeight:700,color:"#8a8780",textTransform:"uppercase",letterSpacing:"0.06em"}}>Account information</div>
      <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
        <Field label="Account name *" half><input style={inpS} value={form.account_name||""} onChange={set("account_name")} placeholder="e.g., Chase Business Checking"/></Field>
        <Field label="Category" half><select style={inpS} value={form.category||""} onChange={set("category")}><option value="">Select...</option>{VAULT_CATS.map(function(c){return <option key={c}>{c}</option>;})}</select></Field>
        <Field label="Website / URL" half><input style={inpS} value={form.url||""} onChange={set("url")} placeholder="e.g., chase.com"/></Field>
        <Field label="Account #" half><input style={inpS} value={form.account_number||""} onChange={set("account_number")}/></Field>
        <Field label="Phone" half><input style={inpS} value={form.phone||""} onChange={set("phone")} placeholder="Customer service number"/></Field>
        <Field label="Contact name" half><input style={inpS} value={form.contact_name||""} onChange={set("contact_name")} placeholder="Account rep or agent"/></Field>
      </div>

      <div style={{fontSize:12,fontWeight:700,color:"#8a8780",textTransform:"uppercase",letterSpacing:"0.06em"}}>Credentials</div>
      <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
        <Field label="Username" half><input style={inpS} value={form.username||""} onChange={set("username")}/></Field>
        <Field label="Email on account" half><input style={inpS} value={form.email||""} onChange={set("email")}/></Field>
        <Field label="Password" half><input style={inpS} value={form.password||""} onChange={set("password")} type={showPw.editpw?"text":"password"}/></Field>
        <div style={{display:"flex",alignItems:"flex-end",marginBottom:2}}><button onClick={function(){togglePw("editpw");}} style={{padding:"5px 10px",borderRadius:6,border:"1px solid #d0cec7",background:"#fff",fontSize:11,cursor:"pointer",color:"#6b6960"}}>{showPw.editpw?"Hide":"Show"}</button></div>
        <Field label="PIN" half><input style={inpS} value={form.pin||""} onChange={set("pin")} type={showPw.editpin?"text":"password"}/></Field>
        <div style={{display:"flex",alignItems:"flex-end",marginBottom:2}}><button onClick={function(){togglePw("editpin");}} style={{padding:"5px 10px",borderRadius:6,border:"1px solid #d0cec7",background:"#fff",fontSize:11,cursor:"pointer",color:"#6b6960"}}>{showPw.editpin?"Hide":"Show"}</button></div>
        <Field label="Security questions"><textarea style={{...inpS,minHeight:50,resize:"vertical"}} value={form.security_questions||""} onChange={set("security_questions")} placeholder="Q: Mother's maiden name? A: ..."/></Field>
      </div>

      <div style={{fontSize:12,fontWeight:700,color:"#8a8780",textTransform:"uppercase",letterSpacing:"0.06em"}}>Access & notes</div>
      <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
        <Field label="Access level" half><select style={inpS} value={form.access_level||"Admin"} onChange={set("access_level")}>{ACCESS_LEVELS.map(function(l){return <option key={l}>{l}</option>;})}</select></Field>
        <Field label="Assigned to" half><input style={inpS} value={form.assigned_to||""} onChange={set("assigned_to")} placeholder="Who manages this account"/></Field>
        <Field label="Notes"><textarea style={{...inpS,minHeight:60,resize:"vertical"}} value={form.notes||""} onChange={set("notes")}/></Field>
      </div>

      <div style={{display:"flex",justifyContent:"flex-end",gap:8}}>
        <button onClick={function(){if(showNew){setShowNew(false);}else{setEditMode(false);}}} style={btnSec}>Cancel</button>
        <button onClick={saveEntry} disabled={!form.account_name||saving} style={{...btnP,opacity:form.account_name?1:0.5}}>{saving?"Saving...":(detail?"Save changes":"Add account")}</button>
      </div>
    </div>;
  };
  ...
  var renderDetail = function() {
    var e = detail;
    var ac = accessColor(e.access_level);
    return <div style={{display:"flex",flexDirection:"column",gap:12}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
        <div>
          <span style={{fontSize:18,fontWeight:700}}>{e.account_name}</span>
          {e.category&&<span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:"#E6F1FB",color:"#0C447C",fontWeight:600,marginLeft:8}}>{e.category}</span>}
          <span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:ac.bg,color:ac.color,fontWeight:600,marginLeft:8}}>{e.access_level}</span>
        </div>
        <button onClick={startEdit} style={btnSec}>Edit</button>
      </div>

      <div style={{display:"flex",flexWrap:"wrap",gap:20,fontSize:14}}>
        {e.url&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Website</span><br/><a href={(e.url.indexOf("http")===0?e.url:"https://"+e.url)} target="_blank" rel="noopener noreferrer" onClick={function(){if(e.password){navigator.clipboard.writeText(e.password).then(function(){});}}} style={{color:"#185FA5"}}>{e.url}</a>{e.password&&<span style={{fontSize:10,color:"#8a8780",marginLeft:8}}>password copied on click</span>}</div>}
        {e.account_number&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Account #</span><br/>{e.account_number}</div>}
        {e.phone&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Phone</span><br/>{e.phone}</div>}
        {e.contact_name&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Contact</span><br/>{e.contact_name}</div>}
        {e.assigned_to&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Managed by</span><br/>{e.assigned_to}</div>}
      </div>

      {(e.username||e.email||e.password||e.pin)&&<>
        <div style={{fontSize:12,fontWeight:700,color:"#8a8780",textTransform:"uppercase",letterSpacing:"0.06em",marginTop:8}}>Credentials</div>
        <div style={{display:"flex",flexWrap:"wrap",gap:20,fontSize:14,background:"#f7f6f3",borderRadius:10,padding:"12px 16px"}}>
          {e.username&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Username</span><br/>{e.username}<button onClick={function(){navigator.clipboard.writeText(e.username);}} style={{marginLeft:6,padding:"2px 6px",borderRadius:4,border:"1px solid #d0cec7",background:"#fff",fontSize:9,cursor:"pointer",color:"#185FA5"}}>Copy</button></div>}
          {e.email&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Email</span><br/>{e.email}</div>}
          {maskedField("Password", e.password, "pw")}
          {maskedField("PIN", e.pin, "pin")}
        </div>
      </>}

      {e.security_questions&&<>
        <div style={{fontSize:12,fontWeight:700,color:"#8a8780",textTransform:"uppercase",letterSpacing:"0.06em",marginTop:8}}>Security questions</div>
        <div style={{fontSize:13,background:"#f7f6f3",borderRadius:8,padding:"10px 14px",whiteSpace:"pre-wrap"}}>{e.security_questions}</div>
      </>}

      {e.notes&&<>
        <div style={{fontSize:12,fontWeight:700,color:"#8a8780",textTransform:"uppercase",letterSpacing:"0.06em",marginTop:8}}>Notes</div>
        <div style={{fontSize:13,background:"#f7f6f3",borderRadius:8,padding:"10px 14px",whiteSpace:"pre-wrap"}}>{e.notes}</div>
      </>}
    </div>;
  };

  if (loading) return <div style={{padding:20,textAlign:"center",color:"#8a8780"}}>Loading vault...</div>;

  return (<div style={{display:"flex",flexDirection:"column",gap:16}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <span style={{fontSize:16,fontWeight:600,color:"#1a1a1a"}}>The Vault</span>
        <span style={{fontSize:12,color:"#8a8780"}}>{filtered.length} accounts{visible.length<entries.length?" ("+entries.length+" total, "+(entries.length-visible.length)+" restricted)":""}</span>
        {(detail||showNew)&&<button onClick={function(){setDetail(null);setShowNew(false);setEditMode(false);setShowPw({});}} style={{background:"none",border:"none",cursor:"pointer",fontSize:13,color:"#185FA5",fontWeight:600}}>&#8592; Back to list</button>}
      </div>
      {!detail&&!showNew&&userRole==="Owner"&&<button onClick={openNew} style={btnP}>+ Add account</button>}
    </div>

    {!detail&&!showNew&&<>
      <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
        <input style={{...inpS,flex:"1 1 200px",minWidth:150}} placeholder="Search accounts..." value={searchQ} onChange={function(e){setSearchQ(e.target.value);}}/>
        <select style={{...inpS,width:"auto",fontSize:12}} value={filterCat} onChange={function(e){setFilterCat(e.target.value);}}><option value="">All categories</option>{VAULT_CATS.map(function(c){return <option key={c}>{c}</option>;})}</select>
      </div>

      {filtered.length===0&&<div style={{padding:20,textAlign:"center",color:"#8a8780",fontSize:14}}>{searchQ||filterCat?"No accounts match your filters.":"No accounts yet. Click + Add account to start."}</div>}

      {Object.keys(byCat).sort().map(function(cat) {
        return <div key={cat}>
          <div style={{fontSize:12,fontWeight:700,color:"#8a8780",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:6,marginTop:8}}>{cat} ({byCat[cat].length})</div>
          {byCat[cat].map(function(entry) {
            var ac = accessColor(entry.access_level);
            return <div key={entry.id} onClick={function(){openDetail(entry);}} style={{background:"#fff",border:"1px solid #e8e6df",borderRadius:8,padding:"10px 14px",cursor:"pointer",marginBottom:4,transition:"all 0.15s"}} onMouseEnter={function(e){e.currentTarget.style.borderColor="#185FA5";}} onMouseLeave={function(e){e.currentTarget.style.borderColor="#e8e6df";}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div>
                  <span style={{fontSize:14,fontWeight:600}}>{entry.account_name}</span>
                  {entry.username&&<span style={{fontSize:12,color:"#8a8780",marginLeft:8}}>{entry.username}</span>}
                </div>
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  <span style={{fontSize:10,padding:"2px 6px",borderRadius:4,background:ac.bg,color:ac.color,fontWeight:600}}>{entry.access_level}</span>
                </div>
              </div>
            </div>;
          })}
...
function InventoryPortal() {
  ...
  var [saving, setSaving] = useState(false);
  var [searchQ, setSearchQ] = useState("");
  var [filterLoc, setFilterLoc] = useState("");
  var [filterCat, setFilterCat] = useState("");
  var [viewDisposed, setViewDisposed] = useState(false);

  var LOCATIONS = ["Kitchen Area","Front Entry","Conference Room","Show Room","Office Manager Area","Mail Area","Front Bathroom","File Room","Leesa's Office","Hallway","Brittney's Area","Kalee's Area","Open Area / Designers","Geoff's Office","Geoff's Office Closet","Lanie's Equipment","Utility Closet","Other"];
  var CATEGORIES = ["Appliance","Electronics","Furniture","Equipment","Supplies","Personal","Other"];
  var CONDITIONS = ["New","Excellent","Good","Fair","Poor","Disposed"];

  function loadItems() {
    sbGet("inventory", "select=*&order=location.asc,item_name.asc&limit=500").then(function(r) {
      setItems(r || []);
      setLoading(false);
    });
  }
  ...
  var emptyForm = {item_name:"",category:"",location:"",brand:"",model_number:"",serial_number:"",purchase_date:"",purchase_price:"",purchase_source:"",assigned_to:"",condition:"Good",notes:"",active:true};

  function openNew() { setForm(Object.assign({}, emptyForm)); setShowNew(true); setDetail(null); setEditMode(false); }
  function openDetail(item) { setDetail(item); setEditMode(false); setShowNew(false); setForm(Object.assign({}, emptyForm, item, {purchase_price: item.purchase_price || ""})); }
  function startEdit() { setEditMode(true); }
  ...
  function saveItem() {
    ...
    data.purchase_source = data.purchase_source || null;
    delete data.id;
    delete data.created_at;

    if (detail && detail.id) {
      sbUpdate("inventory", detail.id, data).then(function() {
        setSaving(false); setEditMode(false);
        loadItems();
        setDetail(Object.assign({}, detail, data));
      });
    } else {
      sbInsert("inventory", data).then(function(r) {
        setSaving(false); setShowNew(false);
        loadItems();
        if (r) openDetail(r);
      });
  ...
  var filtered = items.filter(function(item) {
    if (viewDisposed ? item.active : !item.active) return false;
    if (filterLoc && item.location !== filterLoc) return false;
    if (filterCat && item.category !== filterCat) return false;
    if (searchQ) {
      var q = searchQ.toLowerCase();
      var searchable = [item.item_name, item.brand, item.model_number, item.serial_number, item.location, item.assigned_to, item.notes].filter(Boolean).join(" ").toLowerCase();
  ...
  filtered.forEach(function(item) {
    var loc = item.location || "Unassigned";
  ...
  var renderForm = function() {
    return <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
        <Field label="Item name *" half><input style={inpS} value={form.item_name||""} onChange={set("item_name")}/></Field>
        <Field label="Category" half><select style={inpS} value={form.category||""} onChange={set("category")}><option value="">Select...</option>{CATEGORIES.map(function(c){return <option key={c}>{c}</option>;})}</select></Field>
        <Field label="Location" half><select style={inpS} value={form.location||""} onChange={set("location")}><option value="">Select...</option>{LOCATIONS.map(function(l){return <option key={l}>{l}</option>;})}</select></Field>
        <Field label="Brand" half><input style={inpS} value={form.brand||""} onChange={set("brand")}/></Field>
        <Field label="Model #" half><input style={inpS} value={form.model_number||""} onChange={set("model_number")}/></Field>
        <Field label="Serial #" half><input style={inpS} value={form.serial_number||""} onChange={set("serial_number")}/></Field>
        <Field label="Purchase date" half><input style={inpS} value={form.purchase_date||""} onChange={set("purchase_date")} placeholder="e.g., 7/2022 or 2023-06-26"/></Field>
        <Field label="Purchase price" half><DollarInput style={inpS} value={form.purchase_price} onChange={function(v){setForm(function(f){return Object.assign({},f,{purchase_price:v});});}}/></Field>
        <Field label="Purchased from" half><input style={inpS} value={form.purchase_source||""} onChange={set("purchase_source")}/></Field>
        <Field label="Assigned to" half><input style={inpS} value={form.assigned_to||""} onChange={set("assigned_to")} placeholder="Employee name"/></Field>
        <Field label="Condition" half><select style={inpS} value={form.condition||""} onChange={set("condition")}>{CONDITIONS.map(function(c){return <option key={c}>{c}</option>;})}</select></Field>
        <Field label="Status" half><select style={inpS} value={form.active===false?"false":"true"} onChange={function(e){setForm(function(f){return Object.assign({},f,{active:e.target.value==="true"});});}}><option value="true">Active</option><option value="false">Disposed / Removed</option></select></Field>
        <Field label="Notes"><textarea style={{...inpS,minHeight:60,resize:"vertical"}} value={form.notes||""} onChange={set("notes")}/></Field>
      </div>
      <div style={{display:"flex",justifyContent:"flex-end",gap:8}}>
        <button onClick={function(){if(showNew){setShowNew(false);}else{setEditMode(false);}}} style={btnSec}>Cancel</button>
  ...
  var renderDetail = function() {
    ...
    return <div style={{display:"flex",flexDirection:"column",gap:12}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
        <div>
          <span style={{fontSize:18,fontWeight:700}}>{item.item_name}</span>
          {item.category&&<span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:"#E6F1FB",color:"#0C447C",fontWeight:600,marginLeft:8}}>{item.category}</span>}
          {!item.active&&<span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:"#f0eeea",color:"#791F1F",fontWeight:600,marginLeft:8}}>DISPOSED</span>}
        </div>
        <button onClick={startEdit} style={btnSec}>Edit</button>
      </div>
      <div style={{display:"flex",flexWrap:"wrap",gap:20,fontSize:14}}>
        {item.location&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Location</span><br/>{item.location}</div>}
        {item.brand&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Brand</span><br/>{item.brand}</div>}
        {item.model_number&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Model #</span><br/>{item.model_number}</div>}
        {item.serial_number&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Serial #</span><br/>{item.serial_number}</div>}
        {item.purchase_date&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Purchased</span><br/>{item.purchase_date}</div>}
        {item.purchase_price&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Price</span><br/>{fmtC(item.purchase_price)}</div>}
        {item.purchase_source&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>From</span><br/>{item.purchase_source}</div>}
        {item.assigned_to&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Assigned to</span><br/>{item.assigned_to}</div>}
        {item.condition&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Condition</span><br/>{item.condition}</div>}
      </div>
      {item.notes&&<div style={{fontSize:13,color:"#6b6960",background:"#f7f6f3",borderRadius:8,padding:"10px 14px",marginTop:4}}>{item.notes}</div>}
    </div>;
  };

  if (loading) return <div style={{padding:20,textAlign:"center",color:"#8a8780"}}>Loading inventory...</div>;

  return (<div style={{display:"flex",flexDirection:"column",gap:16}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <span style={{fontSize:16,fontWeight:600,color:"#1a1a1a"}}>Inventory</span>
        <span style={{fontSize:12,color:"#8a8780"}}>{filtered.length} items</span>
      ...
      {!detail&&!showNew&&<button onClick={openNew} style={btnP}>+ Add item</button>}
    </div>

    {!detail&&!showNew&&<>
      <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
        <input style={{...inpS,flex:"1 1 200px",minWidth:150}} placeholder="Search items..." value={searchQ} onChange={function(e){setSearchQ(e.target.value);}}/>
        <select style={{...inpS,width:"auto",fontSize:12}} value={filterLoc} onChange={function(e){setFilterLoc(e.target.value);}}><option value="">All locations</option>{LOCATIONS.map(function(l){return <option key={l}>{l}</option>;})}</select>
        <select style={{...inpS,width:"auto",fontSize:12}} value={filterCat} onChange={function(e){setFilterCat(e.target.value);}}><option value="">All categories</option>{CATEGORIES.map(function(c){return <option key={c}>{c}</option>;})}</select>
      </div>
      <div style={{display:"flex",gap:6}}>
        <button onClick={function(){setViewDisposed(false);}} style={{padding:"5px 14px",borderRadius:8,border:!viewDisposed?"2px solid #185FA5":"1px solid #d0cec7",background:!viewDisposed?"#E6F1FB":"#fff",color:!viewDisposed?"#0C447C":"#6b6960",fontSize:12,fontWeight:!viewDisposed?600:400,cursor:"pointer"}}>Active ({items.filter(function(i){return i.active;}).length})</button>
        <button onClick={function(){setViewDisposed(true);}} style={{padding:"5px 14px",borderRadius:8,border:viewDisposed?"2px solid #185FA5":"1px solid #d0cec7",background:viewDisposed?"#E6F1FB":"#fff",color:viewDisposed?"#0C447C":"#6b6960",fontSize:12,fontWeight:viewDisposed?600:400,cursor:"pointer"}}>Disposed ({items.filter(function(i){return !i.active;}).length})</button>
      </div>

      {Object.keys(byLocation).sort().map(function(loc) {
        return <div key={loc}>
          <div style={{fontSize:12,fontWeight:700,color:"#8a8780",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:6,marginTop:8}}>{loc} ({byLocation[loc].length})</div>
          {byLocation[loc].map(function(item) {
            return <div key={item.id} onClick={function(){openDetail(item);}} style={{background:"#fff",border:"1px solid #e8e6df",borderRadius:8,padding:"10px 14px",cursor:"pointer",marginBottom:4,transition:"all 0.15s"}} onMouseEnter={function(e){e.currentTarget.style.borderColor="#185FA5";}} onMouseLeave={function(e){e.currentTarget.style.borderColor="#e8e6df";}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div>
                  <span style={{fontSize:14,fontWeight:600}}>{item.item_name}</span>
                  {item.brand&&<span style={{fontSize:12,color:"#8a8780",marginLeft:8}}>{item.brand}</span>}
                  {item.assigned_to&&<span style={{fontSize:11,color:"#185FA5",marginLeft:8}}>{item.assigned_to}</span>}
                </div>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  {item.category&&<span style={{fontSize:10,padding:"2px 6px",borderRadius:4,background:"#f0eeea",color:"#6b6960"}}>{item.category}</span>}
                  {item.serial_number&&<span style={{fontSize:10,color:"#8a8780"}}>S/N: {item.serial_number.slice(-6)}</span>}
                </div>
              </div>
            </div>;
          })}
      ...
      {filtered.length===0&&<div style={{padding:20,textAlign:"center",color:"#8a8780",fontSize:14}}>{searchQ||filterLoc||filterCat?"No items match your filters.":"No inventory items yet. Click + Add item to start."}</div>}
    </>}
...
/* ── Hidden Console (Owner + Admin) ── */
function HiddenConsole({ authUser }) {
  var [tab, setTab] = useState("employees");
  var tabStyle = function(active) { return { padding: "8px 20px", borderRadius: 8, border: active ? "2px solid #185FA5" : "1px solid #d0cec7", background: active ? "#E6F1FB" : "#fff", color: active ? "#0C447C" : "#6b6960", fontSize: 13, fontWeight: active ? 600 : 400, cursor: "pointer" }; };
  return <div>
    <div style={{display:"flex",gap:8,marginBottom:20}}>
      <button onClick={function(){setTab("employees");}} style={tabStyle(tab==="employees")}>Employee Portal</button>
      <button onClick={function(){setTab("inventory");}} style={tabStyle(tab==="inventory")}>Inventory</button>
      <button onClick={function(){setTab("vault");}} style={tabStyle(tab==="vault")}>The Vault</button>
    </div>
    {tab==="employees"&&<EmployeePortal/>}
    {tab==="inventory"&&<InventoryPortal/>}
    {tab==="vault"&&<PasswordVault authUser={authUser}/>}
  </div>;
}
...
function NewProjectModal({ onClose, onCreated }) {
  ...
  return (<Modal title="New Project" onClose={onClose} width={620}>
    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
      ...
      <Field label="Salesperson" half><select style={inpS} value={pf.salesperson} onChange={setP("salesperson")}>{ACTIVE_TEAM.map(t=><option key={t}>{t}</option>)}</select></Field>
...
function AuthenticatedApp({ authUser, onLogout }) {
  ...
  const loadProjects = useCallback(() => {
    ...
    if (fSalesperson) { const spFilter = "salesperson=eq." + encodeURIComponent(fSalesperson); params+="&"+spFilter; countP+="&"+spFilter; }
  ...
  }, [page, sortCol, sortDir, fStage.join(","), fType.join(","), fLoc.join(","), dateFrom, dateTo, search, fSalesperson]);
    ...
    {(projectView!=="home"&&projectView!=="contactsview")&&<div>
      ...
      {projectView==="list"&&!isProduction&&<div>
        <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap"}}>
          {(isSales?[["mine","My leads"]]:[]).concat([["active","Active"],["all","All"],["sold","Sold"],["lost","Lost"],["new","New this month"],["estimated","Estimated +"]]).map(function(v) {
