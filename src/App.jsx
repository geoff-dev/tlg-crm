            ...
            {items.map(p => {
              ...
              return (<div key={p.id} onClick={()=>onOpenProject(p)} style={{background:"#fff",borderRadius:8,padding:"8px 10px",border:"1px solid #e8e6df",cursor:"pointer",boxShadow:"0 1px 2px rgba(0,0,0,0.03)",transition:"box-shadow 0.15s",minWidth:155,maxWidth:190,flexShrink:0}} onMouseEnter={e=>{e.currentTarget.style.boxShadow="0 3px 8px rgba(0,0,0,0.08)";}} onMouseLeave={e=>{e.currentTarget.style.boxShadow="0 1px 2px rgba(0,0,0,0.03)";}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:1}}>
                  <div style={{fontWeight:600,fontSize:12,lineHeight:1.3,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",flex:1}}>{p.job_name||"—"}</div>
...
export default function App() {
  ...
  const tableCols = [{k:"client",l:"Client"},{k:"stage",l:"Stage"},{k:"project_type",l:"Type"},{k:"confidence",l:"Confidence"},{k:"lead_score",l:"Lead Score"},{k:"lead_source",l:"Source"},{k:"lead_date",l:"Lead date"},{k:"sale_amount",l:"Sale $"}];
  ...
  return (<div style={{maxWidth:1200,margin:"0 auto"}}>
    ...
    {section==="projects"&&<div>
      ...
      {projectView==="list"&&<div>
        ...
        {loading?<table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{borderBottom:"1px solid #e8e6df",textAlign:"left"}}>{tableCols.map(col=><th key={col.k} style={{padding:8,fontWeight:700,color:"#8a8780",fontSize:12}}>{col.l}</th>)}</tr></thead><tbody>{Array.from({length:10}).map((_,i)=><SkeletonRow key={i} cols={8}/>)}</tbody></table>:
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            ...
            <tbody>
              {projects.length===0&&<tr><td colSpan={8} style={{padding:40,textAlign:"center",color:"#b0ada6"}}>{hasF?"No projects match.":"No projects found."}</td></tr>}
              {projects.map(p=>{const ct=projectContacts[p.contact_id]; const clientName=ct?`${ct.first_name||""} ${ct.last_name||""}`.trim():"";
