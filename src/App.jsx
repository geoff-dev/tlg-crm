...
function GoalsSettings() {
  ...
  function setGoalField(cat, name, field, val) {
    const key = `${cat}:${name}`;
    setGoals(prev => ({ ...prev, [key]: { ...getGoal(cat, name), [field]: val } }));
  }
  ...
  const cellS = { padding: "4px 6px", borderRadius: 6, border: "1px solid #e0ded8", fontSize: 13, width: 90, textAlign: "right", background: "#fff" };
  const thS = { padding: "6px 8px", textAlign: "right", color: "#8a8780", fontSize: 11, fontWeight: 700, textTransform: "uppercase" };
  const tdS = { padding: "4px 8px" };

  function GoalRow({ cat, name, label }) {
    const g = getGoal(cat, name);
    return (
      <tr style={{ borderBottom: "1px solid #f0eeea" }}>
        <td style={{ ...tdS, fontWeight: 500, fontSize: 13 }}>{label || name}</td>
        <td style={tdS}><input style={cellS} value={g.lead_goal || ""} onChange={e => setGoalField(cat, name, "lead_goal", e.target.value)} placeholder="0" /></td>
        <td style={tdS}><input style={cellS} value={g.sales_goal || ""} onChange={e => setGoalField(cat, name, "sales_goal", e.target.value)} placeholder="0" /></td>
        <td style={tdS}><input style={cellS} value={g.revenue_goal || ""} onChange={e => setGoalField(cat, name, "revenue_goal", e.target.value)} placeholder="$0" /></td>
      </tr>
    );
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody><GoalRow cat="annual" name="Annual" label={`${year} Total`} /></tbody>
        </table>
      </div>
  ...
  useEffect(() => {
    Promise.all([
      ...
      contactPromise.then(contacts => {
        ...
        setData({ all, sold, lost, active, contacts, monthly });
      });
    });
  }, [year]);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        ...
        <span style={{ fontSize: 13, color: "#8a8780" }}>
          {data.sold.length} sold · {data.lost.length} lost · {data.active.length} active · {fmtC(data.sold.reduce((s, p) => s + (parseFloat(p.sale_amount) || 0), 0))} revenue
        </span>
      </div>
...
function drawScene(cx, W, H, data, goals, year) {
  ...
  cx.fillText("Leaves = sold  |  Forest = active leads  |  Gravestones = lost (size by stage)  |  " + pctText, 680, 50);

  // Legend
  cx.fillStyle = "rgba(0,0,0,0.3)"; cx.beginPath(); const lx = 30, ly = H - 60, lw = W - 60, lh = 45;
  ...
  cx.fillText("Green trees = 0-30d  |  Yellow = 31-60d  |  Orange = 61-90d  |  Red = 91d+  |  Height = estimate $  |  Small gravestones = early death, large = late stage", 60, ly + 35);
}
