...
function EmployeePortal() {
  ...
  var emptyForm = {name:"",home_address:"",home_city:"",home_state:"IN",home_zip:"",personal_email:"",personal_phone:"",company_cell:"",cell_model:"",phone_last_updated:"",hire_date:"",birthdate:"",compensation_type:"",compensation_amount:"",emergency_contact_name:"",emergency_contact_relationship:"",emergency_contact_phone:"",dl_last4:"",dl_state:"IN",dl_expiration:"",termination_date:"",termination_reason:"",termination_notes:"",active:true,cards:[]};
  ...
  function openDetail(emp) { setDetail(emp); setEditMode(false); setShowNew(false); setForm(Object.assign({}, emptyForm, emp, {compensation_amount: emp.compensation_amount || "", cards: emp.cards || []})); }
  ...
  var renderForm = function() {
    return <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
        <Field label="Full name *" half><input style={inpS} value={form.name||""} onChange={set("name")}/></Field>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
        ...
        <Field label="Last updated" half><input type="date" style={inpS} value={form.phone_last_updated||""} onChange={set("phone_last_updated")}/></Field>
      </div>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
        ...
        <Field label="Phone" half><input style={inpS} value={form.emergency_contact_phone||""} onChange={set("emergency_contact_phone")}/></Field>
      </div>

      <div style={{fontSize:12,fontWeight:700,color:"#8a8780",textTransform:"uppercase",letterSpacing:"0.06em"}}>Driver's license</div>
      <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
        <Field label="Last 4 of DL #" half><input style={inpS} value={form.dl_last4||""} onChange={set("dl_last4")} maxLength={4}/></Field>
        <Field label="State" half><input style={inpS} value={form.dl_state||""} onChange={set("dl_state")}/></Field>
        <Field label="Expiration" half><input type="date" style={inpS} value={form.dl_expiration||""} onChange={set("dl_expiration")}/></Field>
      </div>
      ...
      <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:8}}>
        ...
        <button onClick={saveEmployee} disabled={!form.name||saving} style={{...btnP,opacity:form.name?1:0.5}}>{saving?"Saving...":(detail?"Save changes":"Create employee")}</button>
      </div>
    </div>;
  };
  ...
  var renderDetail = function() {
    ...
    return <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
        <div><span style={{fontSize:20,fontWeight:700}}>{e.name}</span>{!e.active&&<span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:"#f0eeea",color:"#791F1F",fontWeight:600,marginLeft:8}}>ARCHIVED</span>}</div>
      ...
      {(e.company_cell||e.cell_model)&&<>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:20,fontSize:14}}>
          ...
          {e.phone_last_updated&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Last updated</span><br/>{fmtD2(e.phone_last_updated)}</div>}
        </div>
      </>}
      ...
      {(e.dl_last4||e.dl_expiration)&&<>
        <div style={{fontSize:12,fontWeight:700,color:"#8a8780",textTransform:"uppercase",letterSpacing:"0.06em",marginTop:8}}>Driver's license</div>
        <div style={{display:"flex",flexWrap:"wrap",gap:20,fontSize:14}}>
          {e.dl_last4&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Last 4</span><br/>****{e.dl_last4}</div>}
          {e.dl_state&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>State</span><br/>{e.dl_state}</div>}
          {e.dl_expiration&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Expires</span><br/><span style={{color:new Date(e.dl_expiration+"T12:00:00")<new Date()?"#791F1F":"inherit"}}>{fmtD2(e.dl_expiration)}</span></div>}
        </div>
      </>}
        ...
        return <div key={emp.id} onClick={function(){openDetail(emp);}} style={{background:"#fff",border:"1px solid #e8e6df",borderRadius:10,padding:"14px 16px",cursor:"pointer",transition:"all 0.15s"}} onMouseEnter={function(e){e.currentTarget.style.borderColor="#185FA5";}} onMouseLeave={function(e){e.currentTarget.style.borderColor="#e8e6df";}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              ...
              <div style={{fontSize:12,color:"#8a8780",marginTop:2}}>{emp.hire_date?"Hired "+new Date(emp.hire_date+"T12:00:00").toLocaleDateString("en-US",{month:"short",year:"numeric"}):""}{emp.company_cell?" · "+emp.company_cell:""}</div>
            </div>
...
function AuthenticatedApp({ authUser, onLogout }) {
  ...
  return (<div style={{maxWidth:1200,margin:"0 auto"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12,marginBottom:16,padding:"0 4px"}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <h1 onClick={function(){if(mgmtAccess){setProjectView("employees");setSection("projects");}}} style={{margin:0,fontSize:22,fontWeight:700,letterSpacing:"-0.03em",color:"#1a1a1a",cursor:mgmtAccess?"pointer":"default",userSelect:"none"}}>TLG<span style={{color:"#185FA5",marginLeft:4}}>CRM</span></h1>
