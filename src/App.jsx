...
function fmtPhone(p) { if (!p) return ""; var d = p.replace(/[^0-9]/g, ""); if (d.length === 10) return d.slice(0,3) + "-" + d.slice(3,6) + "-" + d.slice(6); if (d.length === 11 && d[0] === "1") return d.slice(1,4) + "-" + d.slice(4,7) + "-" + d.slice(7); return p.replace(/--+/g, "-"); }
  ...
  return (<div>
    ...
    <div style={{background:"#fff",borderRadius:14,border:"1px solid #e8e6df",padding:"20px 24px",boxShadow:cardShadow}}>
      ...
      {contact&&<><SH>Client</SH><div style={{display:"flex",flexWrap:"wrap",gap:20,fontSize:14}}>
        ...
        {contact.spouse_phone&&<div><span style={{color:"#8a8780",fontSize:12,fontWeight:600}}>Phone</span><br/>{fmtPhone(contact.spouse_phone)}</div>}
      </div></div>}</>}
...
function NewProjectModal({ onClose, onCreated }) {
  ...
  const handleSave = () => { setSaving(true); cacheClear();
    ...
    else { sbInsert("contacts",cf).then(contact=>{ if(!contact){setSaving(false);return;} const jn=pf.job_name||(cf.last_name||"New")+" Residence"; var proj=cleanProject(pf,contact.id,jn); if(cf.location) proj.job_location=cf.location; sbInsert("projects",proj).then(r=>{setSaving(false);if(r&&onCreated)onCreated(r);}); }); }
  };
