...
function SalesHistory() {
  ...
  useEffect(function() {
    sbGet("projects", "select=id,stage,lead_date,sale_amount,date_sold,project_type,job_location&lead_date=gte." + curYear + "-01-01&lead_date=lte." + curYear + "-12-31&limit=50000").then(function(projs) {
      ...
      setLiveData({ leads: all.length, sold: sold.length, rev: Math.round(rev), avg: sold.length > 0 ? Math.round(rev / sold.length) : 0, close: all.length > 0 ? Math.round(sold.length / all.length * 1000) / 10 : 0, monthly: monthly, byType: byType, byLoc: byLoc, byPrice: byPrice });
    });
  }, []);
  ...
  var HIST_LOC_MAP = {"Indianapolis North":"Marion Co - NE","Indianapolis SE":"Marion Co - SE","Indianapolis NW":"Marion Co - NW","Indianapolis SW":"Marion Co - SW","Carmel / Westfield":"Carmel","Fishers / Noblesville":"Fishers","Hamilton County":"Hamilton Co","Johnson County":"Johnson Co","Hendricks County":"Hendricks Co","Hancock County":"Hancock Co","Other":"Other County / Out of State"};

  var medalColors = [{bg:"#FFF7E0",border:"#D4A017",label:"#8B6914",icon:"🥇"},{bg:"#F5F5F5",border:"#A0A0A0",label:"#666",icon:"🥈"},{bg:"#FFF0E0",border:"#CD7F32",label:"#8B5E14",icon:"🥉"}];
  function getMedalRank(rows, valueKey) {
    var sorted = rows.filter(function(r){return !r.live && (r[valueKey]||0) > 0;}).slice().sort(function(a,b){return (b[valueKey]||0)-(a[valueKey]||0);});
    var ranks = {};
  ...
  function renderDrillTable() {
    ...
    if (drillType === "monthly") {
      ...
      var ranks = getMedalRank(tableRows, "rev");
        ...
        <div style={{overflowX:"auto",maxHeight:500,overflowY:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            ...
            <tbody>{tableRows.filter(function(r){return r.leads>0||r.sold>0||r.rev>0;}).map(function(r,i){
              var medal = ranks[r.year]; var mc = medal !== undefined ? medalColors[medal] : null;
              return <tr key={r.year} style={{borderBottom:"0.5px solid #f0eeea",background:mc?mc.bg:stripeBg(i),borderLeft:mc?"3px solid "+mc.border:"3px solid transparent"}}>
                <td style={Object.assign({},yearTd,r.live?{color:"#0C447C"}:mc?{color:mc.label}:{})}>{mc?mc.icon+" ":""}{r.year}{r.live?" (live)":""}</td>
                <td style={tdS}>{r.leads}</td>
                <td style={tdBold}>{r.sold}</td>
                <td style={Object.assign({},tdBold,mc?{color:mc.label}:{})}>{fmtC(r.rev)}</td>
    ...
    if (drillType === "location") {
      ...
      if (liveData && liveData.byLoc[locCrmName]) {
        var ld = liveData.byLoc[locCrmName];
        tableRows.unshift({ year: curYear, leads: ld[0], sold: ld[1], live: true });
      }
      var ranks = getMedalRank(tableRows, "sold");
      return <div style={{marginTop:16,background:"#fff",border:"1px solid #e8e6df",borderRadius:10,padding:16}}>
        <div style={{fontSize:14,fontWeight:500,marginBottom:12}}>{title}</div>
        <div style={{overflowX:"auto",maxHeight:500,overflowY:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            ...
            <tbody>{tableRows.filter(function(r){return r.leads>0||r.sold>0;}).map(function(r,i){
              ...
              return <tr key={r.year} style={{borderBottom:"0.5px solid #f0eeea",background:mc?mc.bg:stripeBg(i),borderLeft:mc?"3px solid "+mc.border:"3px solid transparent"}}>
                <td style={Object.assign({},yearTd,r.live?{color:"#0C447C"}:mc?{color:mc.label}:{})}>{mc?mc.icon+" ":""}{r.year}{r.live?" (live)":""}</td>
                <td style={tdS}>{r.leads}</td>
                <td style={Object.assign({},tdBold,mc?{color:mc.label}:{})}>{r.sold}</td>
              </tr>;
            })}</tbody>
          </table>
        </div>
    ...
    if (drillType === "price") {
      ...
      if (liveData && liveData.byPrice[drillKey]) {
        var ld = liveData.byPrice[drillKey];
