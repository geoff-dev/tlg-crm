...
function PhoneInput({ value, onChange, style }) {
  ...
  return React.createElement("input", { style: style, value: local, placeholder: "(317) 555-1234",
...
function ProjectDetail({ project, onBack, onSaved, onOpenContact }) {
  ...
  return (<div>
    ...
    <div style={{background:"#fff",borderRadius:14,border:"1px solid #e8e6df",padding:"20px 24px",boxShadow:cardShadow}}>
      ...
      </div>{(contact.spouse_name||contact.spouse_email||contact.spouse_phone)&&<div style={{marginTop:12}}><div style={{fontSize:12,fontWeight:600,color:"#8a8780",marginBottom:4}}>Additional Contact</div><div style={{display:"flex",flexWrap:"wrap",gap:20,fontSize:14}}>
...
function DuplicateWarning({ lastName, email, phone, address, onUseExisting }) {
  ...
  return React.createElement("div", { style: { background: "#FAEEDA", borderRadius: 10, padding: "12px 16px", marginBottom: 14, borderLeft: "4px solid #EF9F27" } },
    React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: "#633806", marginBottom: 8 } }, "⚠ Possible match" + (matches.length > 1 ? "es" : "") + " found — this person may already be in the system:"),
    matches.map(function(c) {
      ...
      return React.createElement("div", { key: c.id, style: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: "1px solid rgba(239,159,39,0.2)" } },
        React.createElement("div", null,
          React.createElement("div", { style: { fontWeight: 600, fontSize: 13, color: "#633806" } }, name),
          React.createElement("div", { style: { fontSize: 12, color: "#8a6a20" } }, details)
        ),
        onUseExisting ? React.createElement("button", { onClick: function() { onUseExisting(c); }, style: Object.assign({}, btnSec, { fontSize: 11, padding: "4px 10px", color: "#633806", borderColor: "#EF9F27" }) }, "Use this contact") : null
      );
    })
  );
}
...
function NewProjectModal({ onClose, onCreated }) {
  ...
  const [saving, setSaving] = useState(false);
  const [cf, setCf] = useState({first_name:"",last_name:"",email:"",phone_cell:"",phone_home:"",address:"",city:"",state:"IN",zip:"",location:"",home_value:"",spouse_name:"",spouse_email:"",spouse_phone:""});
  ...
  return (<Modal title="New Project" onClose={onClose} width={620}>
    ...
    {mode==="new"&&<div style={{marginBottom:16}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
        ...
        <Field label="Additional contact name" half><input style={inpS} value={cf.spouse_name} onChange={setC("spouse_name")}/></Field>
